...
TapeDevice::TapeDevice(const std::string &filename) : filename(filename) {
    ...
    if (!tape) {
        throw TapeReadException(std::string("Failed to open tape: ") +
                                filename);
    }
}
...
void TapeDevice::openTape(const std::string &filename,
...
void processConfigParameters(const nlohmann::json &config) {
    if (config.find("readDelay") == config.end() ||
        config.find("writeDelay") == config.end() ||
        config.find("moveDelay") == config.end() ||
        config.find("rewindDelay") == config.end() ||
...
void TapeDevice::loadConfig(const std::string &configFile) {
    ...
    setConfigParameters(config, readDelay, writeDelay, moveDelay, rewindDelay,
                        tapeLength, maxMemory);
